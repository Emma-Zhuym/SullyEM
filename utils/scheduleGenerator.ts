 ...
 * 让 LLM 生成 slot 时直接绑定 regionId，地图 App 不再靠关键词事后猜。
 */
function buildMapRegionsBlock(world: MapWorld | null): string {
    if (!world || !world.regions?.length) return '';
    const lines = world.regions.map(r => {
        const keys = r.locationKeys?.length ? `（也叫：${r.locationKeys.join('、')}）` : '';
        const desc = r.description ? ` —— ${r.description}` : '';
        return `- id "${r.id}"：${r.name}${desc}${keys}`;
    });
    return `
...
清单里的地点是**"代表地点"**——同类场景归到最接近的一个：出门吃饭/约会 → 餐厅类地点、撸铁跑步 → 健身房类地点。location 照写具体店名（如"烧鸟店"），regionId 填那个同类代表地点的 id。
实在归不进任何一类的（商场、朋友家、路上……）才把 regionId 留空、location 写实际地点名。**绝不要因为归不了类就把 location 硬写成"家"或其他不相关地点**（比如明明出门约会吃饭却写成"家"）。
`;
}
...
- emoji: 一个匹配的emoji
- availability: 这个时段角色能不能看手机回消息？三选一：
  - "online"：闲着、能随时回（默认）
  - "busy"：在忙但偶尔能瞄一眼手机（比如开会、排练、上课、工作）
  - "offline"：完全不可能碰手机（比如睡觉、正式演出、考试、手术、直播、比赛）
  判断标准：如果真人在做这件事时手机响了，ta 会怎么办？马上回→online，等一会儿偷偷看→busy，完全不可能看→offline
- location: 这个时段角色人在哪（2-6字地点名，如"家""公司""健身房"；说不清就省略）${mapRegionsBlock ? `
- regionId: 若该时段发生在下方「角色的地图地点」清单中的某处，填该地点的 id（只能用清单里出现过的 id，不许编造）` : ''}
- innerThought: （可选）挑其中 2-3 个最有戏的时段，写一句 20-50 字的第一人称内心独白（该时段 ta 心里正在想什么，语气贴角色）
${mapRegionsBlock}
...
{
  "slots": [
    { "startTime": "08:00", "activity": "活动名称", "description": "简短描述", "emoji": "🏃", "availability": "online", "location": "地点名", "regionId": "清单里的id(没有则省略)", "innerThought": "这一时段的内心独白(可选)" },
    ...
  ],
  "flowNarrative": {
    "morning": "上午的意识流独白...",
...
- emoji: 一个匹配的emoji
- availability: 角色此刻能不能回消息？"online"（随时能回）/ "busy"（在忙、偶尔能回）/ "offline"（完全不响应）。AI 角色大多数时段是 online 或 busy（在专注处理数据/上网冲浪时），一般不会 offline 除非设定里有"休眠""关机"之类的状态
...
{
  "slots": [
    { "startTime": "08:00", "activity": "状态名", "description": "简短描述", "emoji": "💭", "availability": "online" },
    ...
  ],
  "flowNarrative": {
    "morning": "上午的意识流独白...",
...
export async function generateDailyScheduleForChar(
    ...
    if (style !== 'mindful') {
        mapWorld = await getWorldForChar(char.id).catch(() => null);
    }
    const mapRegionsBlock = buildMapRegionsBlock(mapWorld);
    // [EM-END: map-region-id]
    ...
    try {
        ...
        const slots: ScheduleSlot[] = (parsed.slots || []).map((s: any) => ({
            ...
            emoji: s.emoji,
            location: s.location,
            // [EM-START: map-region-id] 只收清单里真实存在的 id，防 LLM 幻觉
            regionId: typeof s.regionId === 'string' && mapWorld?.regions.some(r => r.id === s.regionId)
