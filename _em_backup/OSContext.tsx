...
export const OSProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  /** 切换当前私聊角色时同步持久化，避免刷新/HMR/Provider 重挂后仍用旧的「上次角色」误回到 Sully */
  ...
  useEffect(() => {
      ...
      // 本会话内防 React Strict Mode 双次 effect：先占坑，避免两次都通过 localStorage 检查前并发排程
      const sessKey = `morning_greet_scheduling_${todayStr}`;
      try {
          if (sessionStorage.getItem(sessKey) === '1') return;
          sessionStorage.setItem(sessKey, '1');
      } catch {
          /* 私密模式等无 sessionStorage 时忽略 */
      }

      localStorage.setItem('morning_greeting_date', todayStr);

      const timeStr = `${String(hour).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
      const userNm = userProfile?.name || '你';
      const fallbacks = [
          `早安呀～${userNm}，今天也要好好过哦。`,
          `早！${userNm}，刚醒就想跟你说一声早安。`,
          `早上好，${userNm}，愿你今天顺心一点。`,
      ];

      /** 早安至少要有像样的一句，避免「老婆，亲」这种半截被接口截断或模型敷衍 */
      const isGoodMorningText = (t: string) => {
      ...
      selected.forEach(async (char, i) => {
          ...
          let content = fallbacks[i % fallbacks.length];
                  ...
                  const resp = await fetch(`${cfg.baseUrl.replace(/\/+$/, '')}/chat/completions`, {
                      ...
                      body: JSON.stringify({
                          ...
                          messages: [
                              ...
                              { role: 'user', content: `现在是早上${timeStr}，你想主动给${userNm}发一条早安消息。请根据你的性格写**一整句**（12～40字，口语自然，不要引号，不要有角色名前缀，不要只输出两三个字）。` },
                          ],
                          temperature: 0.9,
                          // 显式给足 completion：不少中转在未传 max_tokens 时默认极小，中文会在两三个词处被截断
                          max_tokens: 256,
                      }),
                  });
                  if (resp.ok) {
                      ...
                      if (fr === 'length') {
                          console.warn('[Morning] 早安生成 hit finish_reason=length，可能被截断，已优先用兜底句');
                      }
                      const text = data.choices?.[0]?.message?.content?.trim().replace(/^["'「『]|["'」』]$/g, '');
                      if (text && isGoodMorningText(text)) content = text.slice(0, 120);
                  }
              } catch (e) { /* 降级用 fallbacks */ }
          }
  ...
  // 仅依赖 isDataLoaded：避免 characters 每次引用变化重复跑 effect（已由 localStorage + sessionStorage 防重）
          ...
          try {
              ...
              const data = await safeFetchJson(`${baseUrl}/chat/completions`, {
                  ...
                  body: JSON.stringify({
                      ...
                      temperature: 0.85,
                      stream: false,
                      // 必须显式设置：与主聊天不同，此处未传时很多 API 默认 completion 极短，会得到「老婆，亲」类半截句
                      max_tokens: 512,
                  }),
              });
              ...
              if (finishReason === 'length') {
                  console.warn(`[Proactive] ${char.name} 主动消息 finish_reason=length，输出可能被 max_tokens 截断`);
              }
              ...
              // 半截句（接口截断/模型敷衍）：总长太短不像完整一句，用兜底避免「老婆，亲」式残句
              let compactLen = aiContent.replace(/\s/g, '').length;
              if (finishReason === 'length' && compactLen > 0 && compactLen < 28) {
                  console.warn(`[Proactive] ${char.name} 因 length 截断且过短，改用兜底句`);
                  aiContent = `${userName}，我刚拿起手机又想起你啦～`;
                  compactLen = aiContent.replace(/\s/g, '').length;
              }
              if (compactLen > 0 && compactLen < 10) {
                  aiContent = `${userName}，我刚拿起手机又想起你啦～`;
              }
