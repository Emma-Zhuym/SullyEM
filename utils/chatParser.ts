 ...
 * 应在「首轮 API 返回后」尽早调用，避免后续 SEARCH/Recall 二次请求覆盖 aiContent 导致定时标记丢失。
 */
export async function stripAndSaveScheduledMessages(
    content: string,
    charId: string,
    charName: string,
    ...
    const matches: { full: string; timeStr: string; msgContent: string }[] = [];
    while ((m = re.exec(content)) !== null) {
        matches.push({ full: m[0], timeStr: m[1].trim(), msgContent: m[2].trim() });
    }
    for (const row of matches) {
        ...
        try {
            ...
            console.log('Notification schedule skipped (web mode)');
        }
        addToast(`${charName} 打算一会儿过来找你`, 'info');
        stripRow();
    }
    return c.replace(/\n{3,}/g, '\n\n').trim();
}
...
export const ChatParser = {
    ...
    sanitize: (text: string): string => {
        return text
            ...
            .replace(/\[{1,2}\s*schedule_message[\s\S]*?\]{1,2}/gi, '')
