...
const Launcher: React.FC = () => {
  ...
  const { openApp, virtualTime, characters, activeCharacterId, theme, lastMsgTimestamp, isDataLoaded, unreadMessages } = useOS();
  ...
  // Calculate total unread count across all chars for dock badge
                     ...
                     <div className="absolute inset-0 flex items-center p-4 gap-4">
                         ...
                         <div className="flex-1 min-w-0 flex flex-col justify-center gap-1">
                             <div className="flex items-center gap-2">
                                 <h3 className="text-lg font-bold text-white tracking-wide drop-shadow-md truncate" style={{ color: contentColor }}>
                                 ...
                                 <div className="px-1.5 py-0.5 bg-white/20 rounded text-[9px] font-bold uppercase tracking-wider" style={{ color: contentColor }}>
                                     {widgetUnread > 0 ? 'NEW MESSAGE' : 'Active'}
                                 </div>
                             </div>
                             
                             <div className="relative">
                                 <div className="text-xs line-clamp-2 font-medium leading-relaxed opacity-90 group-hover:opacity-100 transition-opacity" style={{ color: contentColor }}>
               ...
               {dockAppsConfig.map(app => (
                   <div key={app.id} className="relative">
                        ...
                        {app.id === 'chat' && totalUnread > 0 && (
                            <div className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full text-white text-[9px] flex items-center justify-center border-2 border-white/20 shadow-sm font-bold pointer-events-none">
