...
export const OSProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // 一次性迁移：EM 旧版桌面排序(localStorage appOrder) → 上游 7.22 布局系统(theme.launcherAppOrder)
  // 迁移后删旧 key；上游系统成为唯一排序来源，EM 拖拽已退役（2026-07-22 merge 时上游收编了此功能）
  useEffect(() => {
    if (!isDataLoaded) return;
    try {
      const legacy = localStorage.getItem('appOrder');
      if (legacy && !(theme.launcherAppOrder && theme.launcherAppOrder.length)) {
        const order = JSON.parse(legacy) as string[];
        if (Array.isArray(order) && order.length) updateTheme({ launcherAppOrder: order });
