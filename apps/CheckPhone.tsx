...
// ============================================================
const StatusStrip: React.FC<{ dark?: boolean }> = ({ dark = false }) => {
    const clock = new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
    const color = dark ? 'text-white/70' : 'text-stone-500';
    return (
        <div className="shrink-0" style={{ paddingTop: 'var(--safe-top)' }}>
            <div className={`h-9 flex justify-between px-6 items-center z-30 relative pt-2 ${color}`}>
...
const CheckPhone: React.FC = () => {
    ...
    // Gallery widget images (for home screen photo widget)
    const [galleryPhotos, setGalleryPhotos] = useState<GalleryImage[]>([]);
    ...
    // Load gallery images for photo widget
    useEffect(() => {
        if (targetChar?.id) {
...
${layoutHint[layout || 'generic']}`;
                ...
                if (type === 'chat') {
                    const totalCount = 3;
                    // 置顶联系人：每轮随机抽 0～min(3, 置顶总数) 个，保证不被随机生成漏掉；其余仍走真实角色/虚构随机
                    const pinnedContacts = myContacts.filter(c => c.pinned);
                    const numPinnedThisRound = pinnedContacts.length === 0
                        ? 0
                        : 1 + Math.floor(Math.random() * Math.min(3, pinnedContacts.length));
                    const selectedPinned = numPinnedThisRound <= 0
                        ? []
                        : [...pinnedContacts].sort(() => Math.random() - 0.5).slice(0, Math.min(numPinnedThisRound, totalCount));
                    const pinnedList = selectedPinned.map(c => c.identity ? `${c.name}（${c.identity}）` : c.name).join('、');
                    const pinnedSection = selectedPinned.length > 0
                        ? `\n**置顶联系人（本轮必须包含）**：请为以下 ${selectedPinned.length} 位置顶联系人各生成一段对话：${pinnedList}。"title" 必须填这些人的**原名**（与上面列表一致），"kind"/"linkedName"/"identity" 按 TA 的真实身份填（若是神经链接里的真实角色则 kind=real）。其余 ${totalCount - selectedPinned.length} 条按下面规则正常生成。\n`
                        : '';

                    promptInstruction = `生成 ${totalCount} 个**你（${targetChar.name}）自己**手机聊天软件(Message/Line)里的**对话片段**（你和你自己联系人的对话，第一人称视角，不是用户的社交）。
...
返回 JSON：{ "title": "剧情标题(12字内)", "transcript": "我: ...\\n对方: ..." }`;
    ...
    // 置顶：每轮「刷新消息列表」保证抽中 0~3 个置顶联系人，不被随机生成漏掉
    const handleTogglePin = (contact: PhoneContact) => {
        mutateContacts(cs => cs.map(c => c.id === contact.id ? { ...c, pinned: !c.pinned } : c));
        addToast(contact.pinned ? '已取消置顶' : '已置顶：刷新时会优先出现', 'success');
    };
                            ...
                            <div className="flex flex-col items-center gap-2 pt-1">
                                ...
                                <div className="flex items-center gap-2 flex-wrap justify-center">
                                    ...
                                    <button
                                        ...
                                        className={`text-[10px] px-2 py-0.5 rounded-full transition-colors ${c.pinned ? 'bg-amber-400/20 text-amber-300' : 'bg-white/[0.06] text-white/40'}`}
                                        title="置顶：刷新消息列表时保证抽中，不被随机漏掉"
                                    >
                                        📌 {c.pinned ? '已置顶' : '置顶'}
                                    </button>
                                </div>
