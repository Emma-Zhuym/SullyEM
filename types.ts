...
export interface CharacterProfile {
  ...
  hideBeforeMessageId?: number; // New: For history hiding feature
...
export interface StudyChapter {
    ...
    isCompleted: boolean;
    rawContentRange?: { start: number, end: number }; // Optional: simple mapping to text index
    content?: string; // [UPDATED] Store the generated lecture content
}

export interface StudyCourse {
    id: string;
    title: string;
    rawText: string; // The full text extracted from PDF
    chapters: StudyChapter[];
    currentChapterIndex: number;
    createdAt: number;
    coverStyle: string; // CSS gradient
    totalProgress: number; // 0-100
    preference?: string; // [UPDATED] User preference for this course (e.g., "Explain simply")
}
...
export interface FullBackupData {
    ...
    courses?: StudyCourse[]; // Added Courses
